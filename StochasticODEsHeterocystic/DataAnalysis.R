...
t = as.data.frame(c(1:26279)/24)

heterotrophy <- read.csv("52degN_L_troph.csv", head = FALSE)
heterotropht <- read.csv("heterotropht.csv")
heterotrophy_2 <- cbind(months3, t, heterotrophy)
Name <- as.data.frame(rep("High", dim(heterotrophy_2)[1]))
...
spatialy <- read.csv("52degN_L_spatial.csv", head = FALSE)
spatialt <- read.csv("heterotropht.csv")
spatial_2 <- cbind(months3, t, spatialy)
Name2 <- as.data.frame(rep("Med", dim(spatial_2)[1]))
...
temporaly <- read.csv("52degN_L_temporal.csv", head = FALSE)
temporalt <- read.csv("temporalt.csv")
temporal_2 <- cbind(months3, t, temporaly)
Name3 <- as.data.frame(rep("Low", dim(temporal_2)[1]))
...
p+geom_line(size = 0.1) + scale_color_manual(values=GrandBudapest)
